...
const TOOLS = [
  {
    ...
    description:
      "Full-text search across AP decisions, boetes (fines), and aanbevelingen (recommendations). Returns matching decisions with reference, entity name, fine amount, and GDPR articles cited.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: { type: "string", description: "Search query in Dutch (e.g., 'toestemming cookies', 'kinderen', 'TikTok')" },
        type: {
          type: "string",
  ...
  },
  {
    ...
    description: "Get a specific AP decision by reference number (e.g., 'AP-2021-001').",
    inputSchema: {
      type: "object" as const,
      properties: {
        reference: { type: "string", description: "AP decision reference" },
      },
      required: ["reference"],
    },
  },
  {
    ...
    description: "Search AP guidance documents: handleidingen, normuitleg, richtsnoeren, and beleidsregels.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: { type: "string", description: "Search query in Dutch" },
        type: {
          type: "string",
  ...
  },
  {
    ...
    description: "Get a specific AP guidance document by its database ID.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description: "List all covered data protection topics with Dutch and English names.",
...
async function main(): Promise<void> {
  ...
  async function handleRequest(
    ...
    if (url.pathname === "/mcp") {
      ...
      // stack size exceeded" observed in prod logs).
      let closing = false;
      transport.onclose = () => {
        if (closing) return;
        closing = true;
